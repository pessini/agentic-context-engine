...
- `ace/` contains the core library (skillbook, roles, adapters, integrations, observability). See `ace/integrations/` for wrappers (Claude Code, LangChain, LiteLLM, browser-use).
- `tests/` holds unit/integration tests (pytest + unittest support).
- `examples/` contains runnable demos grouped by integration.
- `benchmarks/` and `scripts/` are research/evaluation tooling (not shipped to PyPI).
- `docs/` hosts guides and deeper reference material.

### Build, Test, and Development Commands
- `uv sync` installs all dependencies for contributors (recommended).
- `uv run pytest` runs the pytest suite with coverage and strict markers.
- `python -m unittest discover -s tests` runs the unittest suite.
- `uv run black ace/ tests/ examples/` formats code.
- `uv run mypy ace/` runs type checks.
- `uv run pre-commit install` sets up pre-commit hooks.

### Coding Style & Naming Conventions
- Follow PEP 8 with Black formatting (line length 88).
- Use type hints and docstrings for public APIs.
- Python versions target 3.12 (see `pyproject.toml`).
- Test file naming: `tests/test_*.py`; tests: `test_*` functions, `Test*` classes.

### Testing Guidelines
- Pytest is the primary runner; coverage is enforced with `--cov-fail-under=25`.
- Use markers: `unit`, `integration`, `slow` (e.g., `uv run pytest -m unit`).
- Add tests for new features and include regression coverage when fixing bugs.

### Commit & Pull Request Guidelines
- Recent commit history uses short, imperative summaries (e.g., “Fix …”, “Add …”).
- The contribution guide requests Conventional Commits (`feat(scope): subject`). Prefer Conventional Commits for new work.
- PRs should include a clear description, test results (commands run), and any relevant docs updates (e.g., README/CHANGELOG).
